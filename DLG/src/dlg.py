...
class DLG(nn.Module):
    """Depth-inspired label generation"""
    def __init__(self, resolution, num_slots, in_out_channel=3, iters=5):
        """
        Args:
        ...
        self.in_out_channels = in_out_channel
        ...
        self.decoder_cnn = nn.Sequential(
            ...
            nn.InstanceNorm2d(64, affine=True),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, in_out_channel + 1, kernel_size=5, padding=2, stride=1)
        )
    ...
    def forward(self,  depth):
        x = self.encoder_cnn(depth)  # CNN Backbone.
        ...
        recons, masks = unstack_and_split(x, batch_size=depth.shape[0], num_channels=self.in_out_channels)
        masks = torch.softmax(masks, dim=1)
        recon_combined = torch.sum(recons * masks, dim=1)
        return recon_combined, recons, masks, slots  # [b,3,h,w],[b,2,3,h,w],[b,2,1,h,w],[b,2,c]


if __name__ == '__main__':
    torch.cuda.set_device(0)
    x = torch.rand(4, 3, 256, 256).cuda()
    d = torch.rand(4, 3, 256, 256).cuda()
    model = DLG((256, 256), 2).cuda()
    out = model(d)
